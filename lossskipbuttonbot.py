...
async def generate_prediction(update: Update, data: dict, last_drawn_number: int, user_id: int) -> None:
    ...
    if user_states[user_id]['last_loss']:
        # Switch the category if the last report was a loss
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if user_id exists in user_states, if not, initialize it
    if user_id not in user_states:
        user_states[user_id] = {'category': 'BIG', 'last_loss': False}

    if action == 'win':
        # Do nothing on win, just process silently
        await query.answer("Win, Congratulations 🎉.")
    elif action == 'loss':
        # Update user state to mark last prediction as a loss
        user_states[user_id]['last_loss'] = True
        await query.answer("Next prediction will switch.")

    # We won't edit the message to keep the old prediction
    # Just silently handle the button click
